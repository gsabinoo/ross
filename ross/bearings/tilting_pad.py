...
class THDTilting(BearingElement):
    ...
    def __init__(
        self,
        n,     # Node in which the bearing will be located.
        Rs,    # Rotor radius
        npad,  # Number of pads
        Rp,    # Pad radius
        tpad,  # Pad thickness
        ...
        # Get lubricant properties from ross lubricants dictionary
        ...
        # Extract lubricant properties
        self.rho = self.lubricant_properties["liquid_density"]  # kg/m³
        self.kt = self.lubricant_properties["liquid_thermal_conductivity"]  # W/(m·K)
        self.Cp = self.lubricant_properties["liquid_specific_heat"]  # J/(kg·K)
        
        # Get viscosity interpolation coefficients
        # Convert temperatures from Kelvin back to Celsius
        T1 = self.lubricant_properties["temperature1"] - 273.15  # K to °C
        mu1 = self.lubricant_properties["liquid_viscosity1"]  # Pa·s
        T2 = self.lubricant_properties["temperature2"] - 273.15  # K to °C
        mu2 = self.lubricant_properties["liquid_viscosity2"]  # Pa·s
        
        # Calculate viscosity coefficients
        self.a_a, self.b_b = self._get_interp_coeffs(T1, T2, mu1, mu2)
        
        # Set reference viscosity (mi0) using interpolation at tank temperature
        self.mi0 = self.a_a * np.exp(self.b_b * Tcub)  # Pa·s
    ...
    def p_and_t_solution(self):
        if "impos_EQ" in self.op_key:
            ...
            if "result" in self.choice_CAIMP["impos_EQ"]["print"]:
                ...
                print("-" * 75)
                print(f"{'Total':<5} {'Fx [N]':<18} {'Fy [N]':<22} {'Pads:':<20}")
                print(f"{'':<5} {self.FX_dim:15.6f}     {self.FY_dim:15.6f}     {self.npad:15d}")
                print("="*75)
    ...
    def hde_equilibrium(self, x):
        ...
        # Setup of equilibrium conditions
        eq_0, eq_1, psi_pad = self._setup_equilibrium_conditions(x, n_p)
        
        # Temperature convergence loop
        t_new = self._temperature_convergence_loop(t_new, n_p, eq_0, eq_1, psi_pad)
    ...
    def _setup_equilibrium_conditions(self, x, n_p):
        ...
        return eq_0, eq_1, psi_pad

    def _temperature_convergence_loop(self, t_new, n_p, eq_0, eq_1, psi_pad):
        ...
        while abs((t_new - t_i).max()) >= tol_t:
            ...
            # Calculate viscosity
            mi = self._calculate_viscosity(t_i)
            
            # Assembly and solution of Reynolds equation
            self._solve_reynolds_equation(mi, n_p, eq_0, eq_1, psi_pad)
            
            # Calculate pressure gradients
            self._calculate_pressure_gradients()
            
            # Solve energy equation
            t_new = self._solve_energy_equation(mi, n_p)
        
        return t_new

    def _calculate_viscosity(self, t_i):
        """Calculate dimensionless viscosity based on temperature"""
        mi_i = self.a_a * np.exp(self.b_b * t_i)  # [Pa.s]
        mi = mi_i / self.mi0  # dimensionless viscosity
        return mi

    def _solve_reynolds_equation(self, mi, n_p, eq_0, eq_1, psi_pad):
    ...
    def _calculate_film_thicknesses(self, ii, jj, xr, yr, alpha, n_p):
        ...
        # For north and south faces, we assume the same thickness as the center point
        h_n = h_p
        h_s = h_p
    ...
    def _calculate_h_pt(self, ii, jj, xrpt, yrpt, alphapt, n_p):
        ...
        h_pt = -(1 / (self.Cr_ref[n_p] * self.speed)) * (
            np.cos(self.XTETA[jj]) * xrpt +
            np.sin(self.XTETA[jj]) * yrpt +
            np.sin(self.XTETA[jj]) * (self.Rp + self.tpad) * alphapt
        )
    ...
    def _fill_reynolds_matrix(self, mat_coef, k_idx, ii, jj, c_e, c_w, c_n, c_s, c_p):
        ...
        # Filling based on position in the mesh
        if ii == 0 and jj == 0:
            mat_coef[k_idx, k_idx] = c_p - c_s - c_w
            mat_coef[k_idx, k_idx + 1] = c_e
            mat_coef[k_idx, k_idx + self.nX] = c_n
    ...
    def _solve_energy_equation(self, mi, n_p):
        ...
        for ii in range(self.nZ):
            for jj in range(self.nX):
                # Calculate turbulence and turbulent viscosity
        ...
        mat_coef_t = self._check_diagonal(mat_coef_t)
        t_vec = np.linalg.solve(mat_coef_t, b_t)
        t_new = self._update_temperature_field(t_vec)
    ...
    def _calculate_turbulent_viscosity(self, mi, ii, jj, n_p):
        ...
        self.Reyn[ii, jj, n_p] = (
            self.rho * self.speed * self.Rs * (h_p_loc / self.L) * 
            self.Cr_ref[n_p] / (self.mi0 * mi_p)
        )
    ...
    def _calculate_energy_source_term(self, ii, jj, mi_t, n_p):
        ...
        h_p_loc = self.H[ii, jj]
        h_pt = -(1 / (self.Cr_ref[n_p] * self.speed)) * (
            np.cos(self.XTETA[jj]) * 0 +  # xrpt
            np.sin(self.XTETA[jj]) * 0 +  # yrpt
            np.sin(self.XTETA[jj]) * (self.Rp + self.tpad) * 0  # alphapt
        )
        
        aux_b_t = (self.speed * self.mi0) / (
            self.rho * self.Cp * self.Tcub * self.Cr_ref[n_p]
        )
        
        # Heat generation terms by viscous dissipation
        b_t_g = (
            self.mi0 * self.speed * (self.Rs**2) * self.dX * self.dZ * 
            self.P[ii, jj] * h_pt
        ) / (self.rho * self.Cp * self.T0 * (self.Cr_ref[n_p] ** 2))
        
        b_t_h = (
            self.speed * self.mi0 * (h_pt**2) * 4 * mi_t * self.dX * self.dZ
        ) / (self.rho * self.Cp * self.T0 * 3 * h_p_loc)
        
        b_t_i = aux_b_t * (
            mi_t * (self.Rs**2) * self.dX * self.dZ
        ) / (h_p_loc * self.Cr_ref[n_p])
        
        b_t_j = aux_b_t * (
            (self.Rp**2) * (h_p_loc**3) * (self.dPdX[ii, jj] ** 2) * 
            self.dX * self.dZ
        ) / (12 * self.Cr_ref[n_p] * (self.betha_p**2) * mi_t)
        
        b_t_k = aux_b_t * (
            (self.Rp**4) * (h_p_loc**3) * (self.dPdZ[ii, jj] ** 2) * 
            self.dX * self.dZ
        ) / (12 * self.Cr_ref[n_p] * (self.L**2) * mi_t)
    ...
    def plot_results(self):
        ...
        fig_SP = self.plot_scatter(
            x_data=ang, y_data=self.PPdim, pos=15, y_title="Pressure (Pa)"
        )

        fig_ST = self.plot_scatter(
            x_data=ang, y_data=self.TT_i, pos=15, y_title="Temperature (ºC)"
        )

        fig_CP = self.plot_contourP(
            x_data=ang, y_data=axial, z_data=self.PPdim, z_title="Pressure (Pa)"
        )

        fig_CP = self.plot_contourT(
            x_data=ang, y_data=axial, z_data=self.TT_i, z_title="Temperature (ºC)"
        )
...
def tilting_pad_example02():
    ...
    bearing = THDTilting(
        n = 1,
        Rs = 0.5*101.6e-3, # Seria o journal_diameter (porém, raio no nosso)
        npad = 5, # Número de pads
        Rp = 0.050949799999999996, # Raio da pad (no Max seria Rp = Rs + Cr * (1 + Preload)
        # pre_load = 0, # No max é uma array para cada pad
        tpad = 12.7e-3, # pad_thickness no max
        betha_p = 60, # pad_arc no max (lá em radianos)
        rp_pad = 0.5, # offset no max (posição do pivot)
        L = 50.8e-3, # pad_axial_length do mancal e das pads
        lubricant = "ISOVG32",
        Tcub = 40, # Temperatura do óleo
        nX = 30, # Número de elementos em x
        nZ = 30, # Número de elementos em z
        Cr = 74.9e-6, # radial_clearance no max
        Cr_ref = [74.9e-6, 74.9e-6, 74.9e-6, 74.9e-6, 74.9e-6], # radial_clearance para cada pad (equivalente ao preload)
        sigma = [18, 90, 162, 234, 306], #graus # pivot_angle no max
        speed = 3000, #rpm # frequency no max
        choice_CAIMP = {"impos_EQ":{"pos_EQ":[0.483, 267.5*np.pi/180], # Posição do centro do eixo. No max, você entra com xj e yj
            "ent_angle":[0, 0, 0, 0, 0], # Ângulo de cada pad - Chute inicial
            "print":["result","progress"]}},
            Coefs_D={"b_loc":4, "show_coef":True}
    )
